...
dir_path = f"export/{object.name_full}"
...
for materialSlot in object.material_slots:
    ...
    export_material = {
        ...
        "diffuse_color": f"{material.diffuse_color[0]},{material.diffuse_color[1]},{material.diffuse_color[2]},{material.diffuse_color[3]}",
        "metallic": material.metallic,
        "roughness": material.roughness,
        "specular_color": f"{material.specular_color.r},{material.specular_color.g},{material.specular_color.b}",
    ...
    for node in material.node_tree.nodes:
        if node.type=="TEX_IMAGE":
            ...
            copyfile(imgpath, f"{dir_path}/{os.path.split(imgpath)[1]}")
            export_material["image_paths"].append(os.path.split(imgpath)[1])

    print(json.dumps(export_material,indent=2))

    with open(f"{dir_path}/{material.name_full}.json", 'w') as f:
